...
# -------------------------------------------------
# Stop old clone bot safely
# -------------------------------------------------
async def stop_clone(bot_id: int):
    if bot_id in running_clones:
        old_client = running_clones.pop(bot_id)
        try:
...
# -------------------------------------------------
# Restart a single clone bot
# -------------------------------------------------
async def restart_single_clone(bot_data, start_auto_post: bool = False):
    ...
    try:
        ...
        # 🔹 Start auto-post ONLY if first-time start
        if start_auto_post:
...
# -------------------------------------------------
async def restart_all_clones(start_auto_post: bool = False):
...
# -------------------------------------------------
# Schedule runtime restart (no auto post)
# -------------------------------------------------
async def schedule_clone_restart(hours: int = 6):
    """Restart all clones every X hours, without touching auto-post."""
    while True:
        print("⏳ Scheduled clone restart triggered...")
        try:
            await restart_all_clones(start_auto_post=False)
        except Exception as e:
            print(f"⚠️ Error in scheduled restart: {e}")
...
async def start():
    ...
    # First-time start (with auto post)
    await restart_all_clones(start_auto_post=True)

    # Schedule periodic runtime restarts (no auto post)
